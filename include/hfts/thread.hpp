#ifndef hfts_thread_h
#define hfts_thread_h
...
class Thread {
 public:
  ...
  struct Core {
    ...
    inline bool operator==(Core) const;
    inline bool operator<(Core) const;
  };
  ...
  struct Affinity {
    // supported is true if hfts supports controlling thread affinity for this
    // platform.
#if defined(_WIN32) ||                                                       \
    ...
    class Policy {
     public:
      ...
      virtual Affinity get(uint32_t threadId,
    ...
    Affinity(const containers::vector<Core, 32>&,
                         Allocator* allocator);
