...
class Event {
 public:
  ...
  template <typename Iterator>
  inline static Event any(const Iterator& begin,
                                         const Iterator& end);
//   ///////// test
//   void inc() {
//     shared->count++;
...
//   /////////////////
 ...
 private:
  struct Shared {
    inline Shared(Allocator* allocator,
                                 Mode mode,
                                 bool initialState);
    inline void signal();
    inline void wait();
...
Event::Shared::Shared(Allocator* allocator, Mode mode_, bool initialState)
    : cv(allocator), mode(mode_), signalled(initialState) /*, count(0)*/ {}
