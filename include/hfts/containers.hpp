#ifndef hfts_containers_h
#define hfts_containers_h
...
// Unlike std::vector, hfts::containers::vector keeps the first
...
class vector {
 ...
 private:
  using TStorage = typename hfts::aligned_storage<sizeof(T), alignof(T)>::type;
...
void vector<T, BASE_CAPACITY>::pop_back() {
  //hfts_ASSERT(count > 0, "pop_back() called on empty vector");
  count--;
...
T& vector<T, BASE_CAPACITY>::front() {
  //hfts_ASSERT(count > 0, "front() called on empty vector");
  return reinterpret_cast<T*>(elements)[0];
}

template <typename T, int BASE_CAPACITY>
T& vector<T, BASE_CAPACITY>::back() {
  ////hfts_ASSERT(count > 0, "back() called on empty vector");
...
T& vector<T, BASE_CAPACITY>::operator[](size_t i) {
  //hfts_ASSERT(i < count, "index %d exceeds vector size %d", int(i), int(count));
...
void list<T>::link(Entry* entry, Entry*& list) {
  //hfts_ASSERT(entry->next == nullptr, "link() called on entry already linked");
  //hfts_ASSERT(entry->prev == nullptr, "link() called on entry already linked");
  if (list) {
    entry->next = list;
    list->prev = entry;
  }
