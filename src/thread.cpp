...
struct CoreHasher {
  inline uint64_t operator()(hfts::Thread::Core core) const {
    ...
    for (DWORD i = 0; i < count; i++) {
      if (info[i].Relationship == RelationGroup) {
        ...
        for (WORD groupIdx = 0; groupIdx < groupCount; groupIdx++) {
          ...
          //hfts_ASSERT(out.count <= MaxGroupCount, "Group index overflow");
        }
      }
    }
    return out;
...
Thread::Affinity Thread::Affinity::all(
  ...
  static_assert(!supported,
                "hfts::Thread::Affinity::supported is true, but "
...
Thread::Thread(Func&& func) 
    : impl(new Thread::Impl(
          Thread::Affinity::all(), // 自动创建一个“包含所有核心/无限制”的亲和性对象
          std::move(func)
      )) {}

Thread::~Thread() {
  //hfts_ASSERT(!impl, "Thread::join() was not called before destruction");
}
